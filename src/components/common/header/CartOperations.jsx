...
import styles from './CartOperations.module.css';
...
function CartOperations() {
  ...
  const { logout, isLoggedIn } = useContext(UserAuthContext);

  const navigate = useNavigate();

  const handleOpenPersonOperations = () => {
    setOpenPersonOperations(!openPersonOperations);
  };

  function handleLinkClick() {
    setOpenPersonOperations(false);
  }
  ...
  return (
    <div className={styles.cartOperations}>
      <div className={styles.wishlistIconContainer}>
        ...
        <span className={styles.wishlistItemsNumber}>{wishlistItemCount}</span>
      </div>
      ...
      <div className={styles.cartIconContainer}>
        ...
        <span className={styles.cartItemsNumber}>{totalCartCount}</span>
      </div>
      ...
      {isLoggedIn && (
        <div className={styles.personInfoContainer}>
          <button
            ...
            onClick={handleOpenPersonOperations}
          >
            <svg
              width="32"
              height="32"
          ...
          <div
            className={
              openPersonOperations
                ? styles.personOperations
                : styles.closePersonOperations
            }
          >
            <Link to="/myAccount" onClick={handleLinkClick}>
              ...
              <span>{i18n.t('personOperations.manageMyAccount')}</span>
            </Link>
            <Link to="/cart" onClick={handleLinkClick}>
              ...
              <span>{i18n.t('personOperations.myOrder')}</span>
            </Link>
            <Link onClick={handleLinkClick}>
              ...
              <span>{i18n.t('personOperations.myCancellations')}</span>
            </Link>
            <Link onClick={handleLinkClick}>
              <svg
                width="20"
              ...
              <span>{i18n.t('personOperations.myReviews')}</span>
            </Link>
            ...
            <button onClick={handleLogout}>
              ...
              <span>{i18n.t('personOperations.logout')}</span>
            </button>
          </div>
        </div>
      )}
